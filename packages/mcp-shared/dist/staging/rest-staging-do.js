...
export class RestStagingDO extends DurableObject {
     ...
     */
    migrateMetadata() {
        this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS _do_migrations (
        ...
        if (version < 1) {
            ...
            this.ctx.storage.sql.exec(`CREATE INDEX IF NOT EXISTS idx_session_registry_session_time
					ON _session_registry(session_id, created_at)`);
            this.ctx.storage.sql.exec(`INSERT INTO _do_migrations (id) VALUES (1)`);
        }
    ...
    persistInferredSchema(schema) {
        try {
            ...
            this.ctx.storage.sql.exec(`INSERT OR REPLACE INTO _inferred_schema (id, schema_json) VALUES (1, ?)`, JSON.stringify(merged));
