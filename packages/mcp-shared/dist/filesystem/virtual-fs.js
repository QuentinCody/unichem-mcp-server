...
export class VirtualFS {
    ...
    writeFile(filePath, content) {
        ...
        const existing = this.#sql.exec(`SELECT kind FROM _fs_entries WHERE path = ?`, normalized).toArray();
    ...
    appendFile(filePath, content) {
        ...
        const existing = this.#sql.exec(`SELECT kind, content, size FROM _fs_entries WHERE path = ?`, normalized).toArray();
        if (existing.length > 0 && existing[0].kind === "directory") {
    ...
    readFile(filePath) {
        this.ensureSchema();
        const normalized = this.normalizePath(filePath);
        const rows = this.#sql.exec(`SELECT kind, content FROM _fs_entries WHERE path = ?`, normalized).toArray();
        if (rows.length === 0) {
    ...
    mkdir(dirPath, options) {
        ...
        if (recursive) {
            ...
            const parentRows = this.#sql.exec(`SELECT kind FROM _fs_entries WHERE path = ?`, parent).toArray();
    ...
    readdir(dirPath) {
        this.ensureSchema();
        const normalized = this.normalizePath(dirPath);
        const dirRows = this.#sql.exec(`SELECT kind FROM _fs_entries WHERE path = ?`, normalized).toArray();
        if (dirRows.length === 0) {
        ...
        const rows = this.#sql.exec(`SELECT path FROM _fs_entries WHERE path != ? AND path LIKE ? AND path NOT LIKE ?`, normalized, `${prefix}%`, `${prefix}%/%`).toArray();
    ...
    stat(filePath) {
        this.ensureSchema();
        const normalized = this.normalizePath(filePath);
        const rows = this.#sql.exec(`SELECT path, kind, size, created_at, modified_at FROM _fs_entries WHERE path = ?`, normalized).toArray();
        if (rows.length === 0) {
    ...
    exists(filePath) {
        this.ensureSchema();
        const normalized = this.normalizePath(filePath);
        const rows = this.#sql.exec(`SELECT 1 FROM _fs_entries WHERE path = ?`, normalized).toArray();
    ...
    rm(filePath, options) {
        ...
        const rows = this.#sql.exec(`SELECT kind FROM _fs_entries WHERE path = ?`, normalized).toArray();
        if (rows.length === 0) {
            throw new Error(`Path not found: ${normalized}`);
        }
        const recursive = options?.recursive !== false; // default true
        if (rows[0].kind === "directory") {
            ...
            const children = this.#sql.exec(`SELECT COUNT(*) as c FROM _fs_entries WHERE path LIKE ?`, `${prefix}%`).toArray();
    ...
    glob(pattern) {
        ...
        const rows = this.#sql.exec(`SELECT path FROM _fs_entries WHERE kind = 'file' AND path LIKE ? ESCAPE '\\'`, sqlPattern).toArray();
