...
`;
function bindingsValues(raw, variable) {
    const r = raw;
...
export async function probeSparqlEndpoint(endpointUrl, sparqlFetch) {
    const warnings = [];
    let graphs = [];
    let predicates = [];
    let classes = [];
    try {
        ...
        warnings.push(`VOID graph discovery failed: ${err.message}`);
    }
    try {
        ...
        warnings.push(`Predicate probe failed: ${err.message}`);
    }
    try {
        ...
        warnings.push(`Class probe failed: ${err.message}`);
    }
...
export function buildPrefixHeader(prefixes) {
    return Object.entries(prefixes)
        .map(([k, v]) => `PREFIX ${k}: <${v}>`)
        .join("\n");
}
//# sourceMappingURL=sparql-introspection.js.map